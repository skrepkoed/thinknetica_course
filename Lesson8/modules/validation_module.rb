...
module Validations
  FORBIDDEN_SYMBOLS = /^\W+$/.freeze
  TRAIN_NUMBER = /^[[0-9]|[a-z]]{3}-?[[0-9]|[a-z]]{2}$/.freeze
  ...
  module RailwayValidations
    ...
    def available_carriages?
      available_carriages.size.zero? ? false : true
      # puts InsufficientCarriagesAmount.new.message
    end

    def validate_available_stations(route)
      raise InsufficientStationsAmount if available_stations(route).empty?
    end
